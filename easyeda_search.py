...
from .pcb_injector import PcbInjector
...
class EasyEDASearchDialog(wx.Dialog):
    ...
    def __init__(self, parent):
        ...
        self.pcb_injector = PcbInjector(log_callback=self._log_to_console)
    ...
    def on_delete_instance(self, e):
        ...
        if self.injector.remove_instance_by_uuid(project_path, u_id):
            # Also remove from PCB if placed
            self.pcb_injector.remove_footprint_from_board(board, u_id)
            try:
                pcbnew.Refresh()
            except: pass
    ...
    def on_remove_from_cart(self, e):
        ...
        for i in sorted(indices, reverse=True):
            p = self.cart_items[i]
            # Physically remove all associated instances from schematic and PCB
            for u_id in p.get("placed_uuids", []):
                self.injector.remove_instance_by_uuid(project_path, u_id)
                self.pcb_injector.remove_footprint_from_board(board, u_id)
    ...
    def _download_task(self, lib_name):
        try:
            ...
            sch_path = self.injector.get_sch_path(project_path)
            fps_to_sync = []
            for i, p in enumerate(self.cart_items):
                pcode = p["productCode"]
                qty = p.get("qty", 1)
                ...
                # Queue existing instances for PCB board sync
                for u_id in p["placed_uuids"]:
                    ref_des = self.injector.get_instance_info_by_uuid(project_path, u_id)
                    fps_to_sync.append({
                        "pcode": pcode,
                        "master_fp_dir": master_fp_dir,
                        "ref_des": ref_des or prefix,
                        "friendly_value": p.get("friendlyValue", ""),
                        "u_id": u_id,
                ...
                if needed <= 0 and not fp_missing and lib_exists:
                    self._log_to_console(f"Skipping download for {pcode}: Library up to date.")
                    continue
            ...
            def finish_on_main_thread():
                try:
                    live_board = pcbnew.GetBoard()
                    if live_board:
                        for item in fps_to_sync:
                            self.pcb_injector.inject_footprint_to_board(
                                live_board,
                                item["pcode"],
                                item["master_fp_dir"],
                                item["ref_des"],
                                ...
                                sch_path=item["sch_path"]
                            )
                        pcbnew.Refresh()
                except Exception as ex:
                    self._log_to_console(f"Main thread PCB injection error: {ex}")
    ...
    def _register_project_libraries(self, project_path, lib_name):
        def clean_and_update_table(table_path, name, rel_uri, type_str):
            ...
            if os.path.exists(table_path):
                ...
                version_str = "" if has_version else "  (version 7)\n"
                content = f"({header}\n{version_str}" + "".join(filtered_lines) + new_entry + ")\n"
